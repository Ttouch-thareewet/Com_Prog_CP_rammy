...
def check_neighbours(rows, cols, color,maxrow,maxcol):
  ...
  if rows-1>=0:
    ...
    #call checkNeighbours function from within itself
      check_neighbours(rows-1, cols, color,maxrow,maxcol)
  ...
  if cols-1>=0:
    if matrix[rows][cols-1] ==1 :
      ...
      #call checkNeighbours function from within itself
      check_neighbours(rows, cols-1, color,maxrow,maxcol)
  ...
  for row in range(0,len(matrix)):
    for column in range(0,len(matrix[row])):
      ...
      if matrix[row][column] == 1:
        ...
        check_neighbours(row, column, color,len(matrix),len(matrix[row]))
